...
fn strings_ascii() {
	...
	println!("\nVersion: {}", check_ascii(opus2::version()));
...
fn encode_mono() {
	let mut encoder =
		opus2::Encoder::new(48000, opus2::Channels::Mono, opus2::Application::Audio).unwrap();
...
fn encode_stereo() {
	let mut encoder =
		opus2::Encoder::new(48000, opus2::Channels::Stereo, opus2::Application::Audio).unwrap();
...
fn encode_decode_stereo() {
	let mut opus_encoder =
		opus2::Encoder::new(48000, opus2::Channels::Stereo, opus2::Application::Voip).unwrap();
	let mut opus_decoder = opus2::Decoder::new(48000, opus2::Channels::Stereo).unwrap();
	...
	assert_eq!(MONO_20MS, opus2::packet::get_nb_samples(packet, 48000).unwrap());
	assert_eq!(1, opus2::packet::get_nb_frames(packet).unwrap());
	assert_eq!(opus2::Channels::Stereo, opus2::packet::get_nb_channels(packet).unwrap());
	assert_eq!(MONO_20MS, opus2::packet::get_samples_per_frame(packet, 48000).unwrap());
...
#[test]
fn encode_bad_rate() {
	match opus2::Encoder::new(48001, opus2::Channels::Mono, opus2::Application::Audio) {
		Ok(_) => panic!("Encoder::new did not return BadArg"),
		Err(err) => assert_eq!(err.code(), opus2::ErrorCode::BadArg),
	}
}
...
#[test]
fn repacketizer() {
	let mut rp = opus2::Repacketizer::new().unwrap();
	let mut out = [0; 256];
